    ...
    */
    ...
    while (!finished) {
        ...
        let data = await page.evaluate(() => {
            return Array.from(document.querySelectorAll('tr.af_table_data-row')).map(elem => elem.textContent)
        })
        console.log(data)
        // second page rows
        ...
        let results = data.map(str => {
            ...
            if (str.includes('null')) {
                matches = str.match(/(\d+\/\d+\/\d+).+=null;(.+)([+|-])Amount(.+)Balance(.+)/)
            } else {
